...
export async function POST(req: NextRequest) {
  try {
    ...
    const requestedImages = getNonEmptyImageCount(images)
    const clientIp = getClientIp(req)
    let rateLimit

    try {
      rateLimit = await consumeImageQuotaWithSupabase(supabase, clientIp, requestedImages)
    } catch (rateLimitError: any) {
      if (rateLimitError?.code === 'PGRST202' || rateLimitError?.message?.includes('consume_image_quota')) {
        console.warn('Supabase image quota function unavailable, falling back to in-memory limiter')
        rateLimit = consumeImageQuota(clientIp, requestedImages)
      } else {
    ...
    if (!rateLimit.allowed) {
      return NextResponse.json({
        error: `Hourly image limit exceeded. You can upload up to ${MAX_IMAGES_PER_HOUR.toLocaleString()} images per hour.`,

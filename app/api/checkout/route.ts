...
import { consumeImageQuota, consumeImageQuotaWithSupabase, getClientIp, getNonEmptyImageCount, MAX_IMAGES_PER_HOUR } from '@/utils/imageRateLimit'
...
// Shipping cost tiers by quantity and region
...
// Get shipping cost based on country and quantity
function getShippingCost(country: string, quantity: number): number {
  const tier = SHIPPING_TIERS.find(t => quantity <= t.maxQty) || SHIPPING_TIERS[SHIPPING_TIERS.length - 1]
...
async function saveOrderToDatabase(orderData: any) {
  ...
  try {
    ...
    const insertData = {
      ...
      discount_amount_cents: orderData.discountAmountCents || 0,
      ...
      uploaded_xml_filename: orderData.uploaded_xml_filename || null,
      uploaded_xml_content: orderData.uploaded_xml_content || null,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const shippingCost = getShippingCost(shippingCountry, quantity)
    const normalizedCouponCode = normalizeCouponCode(couponCode || '')
    const subtotalBeforeDiscount = cardsTotal + finishSurcharge

    let claimedCoupon: { id: string; code: string; discount_percent: number } | null = null
    let discountAmount = 0

    // Generate temporary order ID for image organization and coupon claim tracking
    const tempOrderId = `temp_${uuidv4()}`

    if (normalizedCouponCode) {
      if (!supabase) {
        return NextResponse.json({ error: 'Coupons are unavailable because Supabase is not configured.' }, { status: 500 })
      }

      const { data, error } = await (supabase as any).rpc('reserve_one_time_coupon', {
      ...
      const couponRows = data as Array<{ id: string; code: string; discount_percent: number }> | null
      ...
      claimedCoupon = couponRows[0]
      discountAmount = Number(((subtotalBeforeDiscount * claimedCoupon.discount_percent) / 100).toFixed(2))
    }

    const totalAmount = subtotalBeforeDiscount + shippingCost - discountAmount
    ...
    const discountedSubtotalCents = Math.max(0, Math.round((subtotalBeforeDiscount - discountAmount) * 100))
    ...
    if (!imagesAlreadyUploaded) {
      const requestedImages = getNonEmptyImageCount(cardImages || [])
      const rateLimit = supabase
        ? await consumeImageQuotaWithSupabase(supabase, clientIp, requestedImages)
        : consumeImageQuota(clientIp, requestedImages)

      if (!rateLimit.allowed) {
        return NextResponse.json({
          error: `Hourly image limit exceeded. You can process up to ${MAX_IMAGES_PER_HOUR.toLocaleString()} images per hour.`,
          remainingImages: rateLimit.remainingImages,
    ...
    if (claimedCoupon) {
      metadata.couponCode = claimedCoupon.code
      metadata.couponPercent = claimedCoupon.discount_percent.toString()
    }
    ...
    const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = [
      {
        price_data: {
          ...
          product_data: {
            ...
            description: claimedCoupon
              ? `Order subtotal after ${claimedCoupon.discount_percent}% coupon (${claimedCoupon.code})`
              : `Premium S33 cardstock custom cards (${quantity} cards @ $${pricePerCard.toFixed(2)}/card)`,
            images: frontImageUrls.length > 0 && frontImageUrls[0].startsWith('http')
              ? [frontImageUrls[0]]
              : []
          },
          unit_amount: claimedCoupon ? discountedSubtotalCents : Math.round(pricePerCard * 100),
        },
        quantity: claimedCoupon ? 1 : quantity,
      }
    ]

    // Add finish surcharges as a separate line item if any
    if (finishSurchargeCents > 0 && !claimedCoupon) {
      lineItems.push({
        price_data: {
          currency: 'usd',
          product_data: {
    ...
    try {
      ...
      if (claimedCoupon && supabase) {
        await (supabase as any).rpc('release_coupon_claim', {
          p_code: claimedCoupon.code,
          p_order_id: tempOrderId,
        })
      }
      throw stripeError
    ...
    if (supabase) {
      try {
        ...
        const savedOrder = await saveOrderToDatabase({
          ...
          shippingCountry: shippingCountry,
          couponId: claimedCoupon?.id || null,
          couponCode: claimedCoupon?.code || null,
          ...
          uploaded_xml_filename: uploadedXmlFile?.filename || null,
          uploaded_xml_content: uploadedXmlFile?.content || null,
    ...
    if (error?.code === 'PGRST202' || error?.message?.includes('consume_image_quota')) {
      console.error('Supabase image quota function is missing. Run the SQL migration for consume_image_quota before using the limiter.')
    }

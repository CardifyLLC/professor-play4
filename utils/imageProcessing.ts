...
import { canvasToJPGDataURL } from './jpgConverter'
...
export function processImage(
  ...
  img.onload = () => {
    ...
    const DPI = 300
    // Reference dimensions at 300 DPI (63mm × 88mm)
    const refWidth = 750
    const refHeight = 1050

    // Preserve the original image resolution instead of forcing 750×1050
    const baseWidth = img.width // ORIGINAL SIZE PRESERVED
    const baseHeight = img.height // ORIGINAL SIZE PRESERVED

    // Calculate scale factor relative to reference size
    const scaleFactor = Math.max(baseWidth / refWidth, baseHeight / refHeight)

    // Scale mm to pixels based on the image's effective DPI
    const scaledMmToPx = (mm: number) => Math.floor((mm * DPI) / 25.4 * scaleFactor)
    ...
    const activeBleedPx = scaledMmToPx(activeBleedMm)
    ...
    // Fill white background AFTER canvas resize to ensure 24-bit RGB (no alpha channel)
    // This converts any transparency to white, producing JPG output without alpha
    ctx.fillStyle = '#FFFFFF'
    ctx.fillRect(0, 0, canvas.width, canvas.height)

    const trimPx = scaledMmToPx(trimMm)
    ...
    if (addBleed) {
      if (bleedMm > 0) {
        ...
        const dw = baseWidth // Card width (preserves original resolution)
        const dh = baseHeight // Card height (preserves original resolution)

        // Draw the original image at its original size (preserving resolution)
        ctx.drawImage(img, 0, 0, img.width, img.height, dx, dy, baseWidth, baseHeight)
        ...
        // Re-fill white background after canvas resize (resize clears canvas)
        ctx.fillStyle = '#FFFFFF'
        ctx.fillRect(0, 0, canvas.width, canvas.height)

        const cropAmountPx = Math.abs(scaledMmToPx(bleedMm))
        ...
        // bleedMm === 0: No bleed adjustment, preserve original dimensions
        canvas.width = baseWidth
        canvas.height = baseHeight
        ...
        ctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, baseWidth, baseHeight)
      }
    } else {
      // addBleed is false: No bleed processing, preserve original dimensions
      canvas.width = baseWidth
      canvas.height = baseHeight
    ...
    // Export as JPG (24-bit RGB, quality 88%)
    // Fulfills client requirements: JPG format, 85-90 quality, 24-bit RGB, no alpha, sRGB, no metadata
    callback(canvasToJPGDataURL(canvas, 0.88))
  }
...
export function createBlankImage(): string {
  ...
  // Use JPG for blank images too
  return canvasToJPGDataURL(canvas, 0.88).split(',')[1]
}

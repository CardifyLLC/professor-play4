...
export default function CheckoutModal() {
  ...
  const { deck, globalBack, uploadedXmlFile } = useApp()
  ...
  const buildRateLimitMessage = (errorData: any, fallback: string) => {
    const remainingImages = typeof errorData?.remainingImages === 'number' ? errorData.remainingImages : null
    const retryAfterSeconds = typeof errorData?.retryAfterSeconds === 'number' ? errorData.retryAfterSeconds : null

    const retryAfterMinutes = retryAfterSeconds !== null
      ? Math.max(1, Math.ceil(retryAfterSeconds / 60))
      : null

    const details = [
      remainingImages !== null ? `${remainingImages.toLocaleString()} images remaining` : null,
      retryAfterMinutes !== null ? `try again in about ${retryAfterMinutes} minute${retryAfterMinutes === 1 ? '' : 's'}` : null,
    ].filter(Boolean)
  ...
  const getShippingCost = (country: string, qty: number): number => {
    const tiers = [
      { maxQty: 100, US: 6.95, CA: 12.95, INTL: 16.95 },
      { maxQty: 250, US: 8.95, CA: 16.95, INTL: 24.95 },
      { maxQty: 500, US: 12.95, CA: 24.95, INTL: 34.95 },
      { maxQty: 1000, US: 18.95, CA: 34.95, INTL: 54.95 },
      { maxQty: 2000, US: 29.95, CA: 54.95, INTL: 89.95 },
      { maxQty: Infinity, US: 49.95, CA: 89.95, INTL: 149.95 },
    ]
    const tier = tiers.find(t => qty <= t.maxQty) || tiers[tiers.length - 1]
    if (country === 'US') return tier.US
    if (country === 'CA') return tier.CA
    return tier.INTL
  }
  ...
  const shippingCost = getShippingCost(formData.country, quantity)
  ...
  const applyCoupon = async () => {
    ...
    if (!normalizedCode) {
      setErrorMessage('Enter a coupon code to apply it.')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'This coupon is invalid or has already been used.')
      }
      ...
      setErrorMessage(error.message || 'This coupon is invalid or has already been used.')
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cardData = expandedDeck.map((card) => ({
        ...
        xmlTag: card.xmlTag || null, // Preserve XML tag metadata for XML-imported cards
      ...
      if (shouldUploadImagesFirst) {
        ...
        const totalNonEmptyImages = allImages.filter((image) => image && image.trim() !== '').length
        ...
        const maybeUpdateUi = (phase: 'compress' | 'upload') => {
          ...
          if (phase === 'compress') {
            ...
            setProgressText(`Preparing your artwork… (${compressed}/${totalChunks} groups, ${totalNonEmptyImages} images)`)
            ...
            setProgressText(`Uploading images… (${uploaded}/${totalChunks} groups, ${totalNonEmptyImages} images)`)
        ...
        setProgressText(`Preparing your artwork… (0/${totalChunks} groups, ${totalNonEmptyImages} images)`)
        setProgressPct(5)
        setProgressSubtext('We’re optimizing files for a fast, reliable upload—your print quality stays the same.')
        ...
        setProgressText(`Uploading images… (0/${totalChunks} groups, ${totalNonEmptyImages} images)`)
        setProgressPct(50)
        setProgressSubtext('Hang tight—large orders can take a couple minutes. We’re working as fast as we can.')
        ...
        const uploadedChunkResults = await mapWithConcurrency(indices, UPLOAD_CONCURRENCY, async (chunkIdx) => {
          ...
          try {
            ...
            if (!uploadResponse.ok) {
              ...
              // Detect Vercel payload size limit errors and provide a user-friendly message
              const isPayloadTooLarge =
                uploadResponse.status === 413 ||
                (typeof errorText === 'string' && errorText.includes('FUNCTION_PAYLOAD_TOO_LARGE'))
              if (isPayloadTooLarge) {
                throw new Error(
                  'One or more of your images are too large to upload. ' +
                  'Please try using smaller or lower-resolution images (under 5 MB each) and try again.'
                )
              }

              if (uploadResponse.status === 429) {
                throw new Error(buildRateLimitMessage(
                  errorData,
                  'Hourly upload limit reached. You can upload up to 10,000 images per hour'
                ))
              }
        ...
        if (!response.ok) {
          ...
          // Detect Vercel payload size limit on checkout with pre-uploaded URLs
          const isPayloadTooLarge =
            response.status === 413 ||
            (typeof errorText === 'string' && errorText.includes('FUNCTION_PAYLOAD_TOO_LARGE'))
          if (isPayloadTooLarge) {
            throw new Error(
              'Your order data is too large to process. ' +
              'Please try reducing the number of cards or using smaller images and try again.'
            )
          }
          ...
          if (response.status === 429) {
            ...
            throw new Error(buildRateLimitMessage(
              errorData,
              'Hourly image processing limit reached. You can process up to 10,000 images per hour'
            ))
          }
        ...
        if (!response.ok) {
          ...
          // Detect Vercel payload size limit on direct checkout path
          const isPayloadTooLarge =
            response.status === 413 ||
            (typeof errorText === 'string' && errorText.includes('FUNCTION_PAYLOAD_TOO_LARGE'))
          if (isPayloadTooLarge) {
    ...
    <div className="fixed inset-0 bg-slate-900/90 z-[90] flex items-center justify-center backdrop-blur-sm p-2 sm:p-4">
      <div className="bg-white dark:bg-slate-850 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh] animate-modal">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto bg-white dark:bg-slate-850">
          ...
          <div className="mb-4 sm:mb-6 rounded-lg border border-sky-200 bg-sky-50 p-3 sm:p-4 dark:border-sky-900/60 dark:bg-sky-950/30">
            <div className="mb-3 flex items-center gap-2">
              <Ticket className="h-4 w-4 text-sky-600 dark:text-sky-400" />
              <h4 className="font-bold text-sm sm:text-base text-slate-800 dark:text-white">Coupon code</h4>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-sky-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isApplyingCoupon ? 'Checking...' : appliedCoupon ? 'Recheck' : 'Apply'}
                </button>
                {appliedCoupon && (
                  <button
                    type="button"
                    onClick={clearCoupon}
                    className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                  >
                    Remove
                  </button>
                )}
            ...
            <p className="mt-2 text-[11px] text-slate-600 dark:text-slate-400">
              One-time codes apply a 10% discount and are locked after checkout uses them.
            </p>
          </div>

          {/* Error Banner */}
          {errorMessage && (
            <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-red-50 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-800">

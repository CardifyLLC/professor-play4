...
export default function EditorView() {
  ...
  useEffect(() => {
    ...
    const rawBleed = target.hasBleed ? (target.bleedMm !== undefined ? target.bleedMm : 2.0) : 0

    let cutInsetX: number
    let cutInsetY: number
    let aspectW: number
    let aspectH: number

    if (rawBleed > 0) {
      // Positive bleed: canvas expands outward beyond 63×88
      // The image is baseW + 2*bleed wide, and the cut line sits at bleed inset
      aspectW = baseW + (2 * rawBleed)
      aspectH = baseH + (2 * rawBleed)
      cutInsetX = (rawBleed / aspectW) * 100
      cutInsetY = (rawBleed / aspectH) * 100
    } else if (rawBleed < 0) {
      // Negative bleed (crop): the image is zoomed in / cropped.
      // imageProcessing.ts draws the original image scaled to (baseW + 2*|crop|) onto a
      // baseW × baseH canvas, so the visible region is smaller than the full image.
      // The 63×88mm cut boundary sits |crop| mm inside the displayed content.
      // Displayed content maps original (base + 2*|crop|) to the canvas, so inset %:
      const cropMm = Math.abs(rawBleed)
      const contentW = baseW + (2 * cropMm)
      const contentH = baseH + (2 * cropMm)
      aspectW = baseW  // canvas stays at base dimensions
      aspectH = baseH
      cutInsetX = (cropMm / contentW) * 100
      cutInsetY = (cropMm / contentH) * 100
    } else {
      // No bleed adjustment
      aspectW = baseW
      aspectH = baseH
    ...
    canvas.style.aspectRatio = `${aspectW} / ${aspectH}`
    // Ensure canvas is visible
    canvas.style.display = ''
    canvas.style.visibility = 'visible'

    // Position the cut line overlay

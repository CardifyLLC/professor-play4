  ...
  useEffect(() => {
    if (isOpen && activeVersionIndex >= 0 && deck[activeVersionIndex]) {
      ...
      if (card.printsUri) {
        ...
        setVersions([])
        setError('Cards loaded from Scryfall must have 2.0mm bleed. Alternative art versions are only available for cards imported from Scryfall. To change the art for manually uploaded images, please upload a new image via the sidebar.')
      }
    }
